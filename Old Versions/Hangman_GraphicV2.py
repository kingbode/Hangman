...
class main():
    ...
    def __init__(self):
        ...
        def _test(_word):
            # Creates a Tkinter-compatible photo image, which can be used everywhere Tkinter expects an image object.
            # _drawImage(_image)
            messagebox.showinfo("LOSER!!", f"Sorry! You've lost. The word was {_word}")
        ...
        def _update_wins_losses(_wins, _losses):
            self.wins_label.configure(text=_wins)
            self.losses_label.configure(text=_losses)
...
if __name__ == "__main__":
    ...
    # will be presnted by disabling the button .. no more action in here
    # missed_letters = ''
